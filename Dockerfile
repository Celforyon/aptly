...
RUN apt update \
	&& apt install --no-install-recommends --no-install-suggests -y --force-yes inotify-tools gnupg=1.4.18-7+deb8u3 \
	&& apt install --no-install-recommends --no-install-suggests -y --force-yes -tstretch aptly \
