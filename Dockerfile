...
RUN apt-get update \
	&& apt-get install --no-install-recommends --no-install-suggests -y inotify-tools gnupg=1.4.18-7+deb8u3 \
	&& apt-get install --no-install-recommends --no-install-suggests -y -tstretch aptly \
